...
import { createNAgentAPI } from "./exports"
...
export async function activate(context: vscode.ExtensionContext) {
	outputChannel = vscode.window.createOutputChannel("nAgentCoderAI")
	context.subscriptions.push(outputChannel)

	ErrorService.initialize()
	Logger.initialize(outputChannel)
	Logger.log("nAgentCoderAI extension activated")
	...
	// Migrate custom instructions to global nAgentCoderAI rules (one-time cleanup)
	...
	const previousVersion = context.globalState.get<string>("nagentcoderaiVersion")
	...
	vscode.commands.executeCommand("setContext", "nagentcoderai.isDevMode", IS_DEV && IS_DEV === "true")
	...
	try {
		if (!previousVersion || currentVersion !== previousVersion) {
			Logger.log(`nAgentCoderAI version changed: ${previousVersion} -> ${currentVersion}. First run or update detected.`)
			const lastShownPopupNotificationVersion = context.globalState.get<string>("nagentcoderaiLastPopupNotificationVersion")
			...
			if (currentVersion !== lastShownPopupNotificationVersion && previousVersion) {
				...
				const message = `nAgentCoderAI has been updated to v${currentVersion}`
				...
				await context.globalState.update("nagentcoderaiLastPopupNotificationVersion", currentVersion)
			}
			// Always update the main version tracker for the next launch.
			await context.globalState.update("nagentcoderaiVersion", currentVersion)
		}
	} catch (error) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("nagentcoderai.plusButtonClicked", async (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("nagentcoderai.mcpButtonClicked", (webview: any) => {
	...
	const opennAgentCoderAIInNewTab = async () => {
		Logger.log("Opening nAgentCoderAI in new tab")
		...
		const panel = vscode.window.createWebviewPanel(WebviewProvider.tabPanelId, "nAgentCoderAI", targetCol, {
			enableScripts: true,
	...
	context.subscriptions.push(vscode.commands.registerCommand("nagentcoderai.popoutButtonClicked", opennAgentCoderAIInNewTab))
	context.subscriptions.push(vscode.commands.registerCommand("nagentcoderai.openInNewTab", opennAgentCoderAIInNewTab))

	context.subscriptions.push(
		vscode.commands.registerCommand("nagentcoderai.settingsButtonClicked", (webview: any) => {
			const isSidebar = !webview
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("nagentcoderai.historyButtonClicked", async (webview: any) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("nagentcoderai.accountButtonClicked", (webview: any) => {
	...
	*/
	...
	if (IS_DEV && IS_DEV === "true") {
		...
		import("./dev/commands/tasks")
			.then((module) => {
				...
				Logger.log("nAgentCoderAI dev task commands registered")
			})
			.catch((error) => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("nagentcoderai.addToChat", async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
			await vscode.commands.executeCommand("nagentcoderai.focusChatInput") // Ensure nAgentCoderAI is visible and input focused
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("nagentcoderai.addTerminalOutputToChat", async () => {
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					// Add to nAgentCoderAI (Always available)
					const addAction = new vscode.CodeAction("Add to nAgentCoderAI", vscode.CodeActionKind.QuickFix)
					addAction.command = {
						command: "nagentcoderai.addToChat",
						title: "Add to nAgentCoderAI",
						arguments: [expandedRange, context.diagnostics],
					}
					actions.push(addAction)

					// Explain with nAgentCoderAI (Always available)
					const explainAction = new vscode.CodeAction("Explain with nAgentCoderAI", vscode.CodeActionKind.RefactorExtract) // Using a refactor kind
					explainAction.command = {
						command: "nagentcoderai.explainCode",
						title: "Explain with nAgentCoderAI",
						arguments: [expandedRange],
					}
					actions.push(explainAction)

					// Improve with nAgentCoderAI (Always available)
					const improveAction = new vscode.CodeAction("Improve with nAgentCoderAI", vscode.CodeActionKind.RefactorRewrite) // Using a refactor kind
					improveAction.command = {
						command: "nagentcoderai.improveCode",
						title: "Improve with nAgentCoderAI",
						arguments: [expandedRange],
					}
					actions.push(improveAction)

					// Fix with nAgentCoderAI (Only if diagnostics exist)
					if (context.diagnostics.length > 0) {
						const fixAction = new vscode.CodeAction("Fix with nAgentCoderAI", vscode.CodeActionKind.QuickFix)
						fixAction.isPreferred = true
						fixAction.command = {
							command: "nagentcoderai.fixWithnAgentCoderAI",
							title: "Fix with nAgentCoderAI",
							arguments: [expandedRange, context.diagnostics],
						}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("nagentcoderai.fixWithnAgentCoderAI", async (range: vscode.Range, diagnostics: vscode.Diagnostic[]) => {
			// Add this line to focus the chat input first
			await vscode.commands.executeCommand("nagentcoderai.focusChatInput")
			...
			const visibleWebview = WebviewProvider.getVisibleInstance()
			await visibleWebview?.controller.fixWithnAgentCoderAI(selectedText, filePath, languageId, diagnostics)
			telemetryService.captureButtonClick("codeAction_fixWithnAgentCoderAI", visibleWebview?.controller.task?.taskId, true)
		}),
	)

	context.subscriptions.push(
		vscode.commands.registerCommand("nagentcoderai.explainCode", async (range: vscode.Range) => {
			await vscode.commands.executeCommand("nagentcoderai.focusChatInput") // Ensure nAgentCoderAI is visible and input focused
			await pWaitFor(() => !!WebviewProvider.getVisibleInstance())
			const editor = vscode.window.activeTextEditor
			if (!editor) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("nagentcoderai.improveCode", async (range: vscode.Range) => {
			await vscode.commands.executeCommand("nagentcoderai.focusChatInput") // Ensure nAgentCoderAI is visible and input focused
			await pWaitFor(() => !!WebviewProvider.getVisibleInstance())
			const editor = vscode.window.activeTextEditor
			if (!editor) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("nagentcoderai.focusChatInput", async () => {
			...
			if (activeWebviewProvider?.getWebview() && activeWebviewProvider.getWebview().hasOwnProperty("reveal")) {
				...
				if (!activeWebviewProvider) {
					// No existing nAgentCoderAI view found at all, open a new tab
					await vscode.commands.executeCommand("nagentcoderai.openInNewTab")
			...
			if (activeWebviewProvider) {
				...
				console.error("FocusChatInput: Could not find or activate a nAgentCoderAI webview to focus.")
				vscode.window.showErrorMessage(
					"Could not activate nAgentCoderAI view. Please try opening it manually from the Activity Bar.",
				)
			}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("nagentcoderai.generateGitCommitMessage", async () => {
			...
			if (controller) {
				...
				const outputChannel = vscode.window.createOutputChannel("nAgentCoderAI Commit Generator")
	...
	return createNAgentAPI(outputChannel, sidebarWebview.controller)
}
...
export async function deactivate() {
	...
	Logger.log("nAgentCoderAI extension deactivated")
}
