...
import { Controller as nAgentCoderAIProvider } from "@core/controller"
import { fileExistsAtPath } from "@utils/fs"
import { globby } from "globby"

class CheckpointTracker {
	private providerRef: WeakRef<nAgentCoderAIProvider>
	...
	private constructor(provider: nAgentCoderAIProvider, taskId: string, cwd: string) {
	...
	public static async create(
		...
		provider?: nAgentCoderAIProvider,
	...
	private static async getWorkingDirectory(): Promise<string> {
		...
		if (!cwd) {
			throw new Error("No workspace detected. Please open nAgentCoderAI in a workspace to use checkpoints.")
		}
	...
	public static async doesShadowGitExist(taskId: string, provider?: nAgentCoderAIProvider): Promise<boolean> {
	...
	public async initShadowGit(): Promise<string> {
		...
		if (await fileExistsAtPath(gitPath)) {
			...
			await git.addConfig("user.name", "nAgentCoderAI Checkpoint")
