...
export async function downloadMcp(controller: Controller, request: StringRequest): Promise<Empty> {
	try {
		...
		const response = await axios.post<McpDownloadResponse>(
			"https://api.nagentcoderai.bot/v1/mcp/download",
			{ mcpId },
			{
...
- Use "${mcpDetails.mcpId}" as the server name in nagentcoderai_mcp_settings.json.
- Create the directory for the new MCP server before starting installation.
- Make sure you read the user's existing nagentcoderai_mcp_settings.json file before editing it with this new mcp, to not overwrite any existing servers.
