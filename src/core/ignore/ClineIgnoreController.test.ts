import { nAgentCoderAIIgnoreController } from "./nAgentCoderAIIgnoreController"
...
describe("nAgentCoderAIIgnoreController", () => {
	let tempDir: string
	let controller: nAgentCoderAIIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .nagentcoderaiignore file
		await fs.writeFile(
			path.join(tempDir, ".nagentcoderaiignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new nAgentCoderAIIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .nagentcoderaiignore file", async () => {
			const result = controller.validateAccess(".nagentcoderaiignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .nagentcoderaiignore", async () => {
			// Create a new .nagentcoderaiignore with comments
			await fs.writeFile(
				path.join(tempDir, ".nagentcoderaiignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .nagentcoderaiignore gracefully", async () => {
			// Create a new controller in a directory without .nagentcoderaiignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new nAgentCoderAIIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .nagentcoderaiignore", async () => {
			await fs.writeFile(path.join(tempDir, ".nagentcoderaiignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .nagentcoderaiignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".nagentcoderaiignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .nagentcoderaiignore
			controller = new nAgentCoderAIIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .nagentcoderaiignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .nagentcoderaiignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".nagentcoderaiignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".nagentcoderaiignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
