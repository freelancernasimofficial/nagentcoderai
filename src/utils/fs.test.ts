...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "nagentcoderai-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .nagentrules/workflows directory specifically", async () => {
		// Create a test directory structure
		const nagentcoderairulesDirTest = path.join(tmpDir, "nagentcoderairules-test")
		const nagentcoderairulesDirPath = path.join(nagentcoderairulesDirTest, ".nagentrules")

		// Create .nagentrules directory and root files
		await fs.mkdir(nagentcoderairulesDirPath, { recursive: true })
		await fs.writeFile(path.join(nagentcoderairulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(nagentcoderairulesDirPath, "settings.js"), "// settings")

		// Create .nagentrules/other directory and files
		const otherDirPath = path.join(nagentcoderairulesDirPath, "other")
		...
		// Create .nagentrules/workflows directory and files
		const workflowsDirPath = path.join(nagentcoderairulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(nagentcoderairulesDirPath)
		...
		const filteredFiles = await readDirectory(nagentcoderairulesDirPath, [[".nagentrules", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(nagentcoderairulesDirPath, "config.json"),
			path.resolve(nagentcoderairulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(nagentcoderairulesDirPath, [
		...
		const rootOnlyFiles = [path.resolve(nagentcoderairulesDirPath, "config.json"), path.resolve(nagentcoderairulesDirPath, "settings.js")]
